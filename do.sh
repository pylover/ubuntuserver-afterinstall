...
# Determine SSH_PORT
if [ -z "${SSH_PORT:-}" ]; then
  echo "Enter SSH port [default: 1111]:"
  read input_ssh_port
...
if [ -z "${PASSWD_ROOT:-}" ]; then
  echo "Enter root password [default: rootpassword]:"
...
if [ -z "${ADMIN_USERS:-}" ]; then
  echo "Enter admin usernames (space-separated) [default: adminuser]:"
...
if [ -z "${PASSWD_ADMIN:-}" ]; then
  ...
  for usr in ${ADMIN_USERS}; do
    echo "Enter password for ${usr} [default: adminpassword]:"
